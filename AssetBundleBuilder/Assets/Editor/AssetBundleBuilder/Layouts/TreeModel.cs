...
namespace UnityEditor.CustomTreeView
{
	...
	public class TreeModel<T> where T : TreeElement
	{
		...
		public void AddElements (IList<T> elements, TreeElement parent, int insertIndex)
		{
			if (elements == null)
			...
			parent.children.InsertRange(insertIndex, elements.Cast<TreeElement> ());
			foreach (var element in elements)
			{
		...
		public void AddElement (T element, TreeElement parent, int insertIndex)
		{
			if (element == null)
